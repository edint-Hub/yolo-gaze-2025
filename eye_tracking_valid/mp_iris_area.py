...
class MPIrisArea:
    ...
    def valid_with_image(self, img_folder_path: str, visible_save: bool = False):
        ...
        for img_name in img_namd_list:  # file name type : {idx}_{x}_{y}.jpg(png)
            ...
            try:
                ...
                if (
                    int(file_info[0]) == self.cell_x
                    and int(file_info[1]) == self.cell_y
                ):
        ...
        print(
            f"valid success percent : {valid_success_count / len(img_namd_list) * 100:.2f}"
        )
    ...
    def __drawing(
        ...
        cv2.putText(
            frame,
            f"[{cal_text}] Press 'c' to calibrate, 'r' to reset",
            (10, height - 20),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.6,
            (200, 200, 200),
